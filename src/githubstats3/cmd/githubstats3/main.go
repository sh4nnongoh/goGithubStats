...
func main() {
	...
	fs := flag.NewFlagSet("githubstats3", flag.ExitOnError)
	var (
	...
	var (
		service     = githubstats3.NewService(logger, ints, chars)
		endpoints   = githubstats3.NewSet(service, logger, duration, tracer)
		httpHandler = githubstats3.NewHTTPHandler(endpoints, tracer, logger)
		//grpcServer     = githubstats3.NewGRPCServer(endpoints, tracer, logger)
		//jsonrpcHandler = githubstats3.NewJSONRPCHandler(endpoints, logger)
	)
